...
from models.loss import TCLoss, MaskedTCLoss
...
class TransBaseModel(nn.Module):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--np_loss', type=str2bool, default=False, const=True, nargs='?')
        parser.add_argument('--np_sample', type=int, default=8)
        parser.add_argument('--np_window', type=int, default=8)
        parser.add_argument('--np_alpha', type=float, default=0.5)

        parser.add_argument('--np_gap', type=str2bool, default=False, const=True, nargs='?')
    ...
    def __init__(self, args, generator, sos_index):
        ...
        try:
            ...
            exit(1)
        # if self.args.pose_backbone == 'PoseBackboneWrapper':
        #     self.visual_backbone = PoseBackboneWrapper()
        # elif self.args.pose_backbone == 'SlidingWindowPoseBackbone':
        #     self.visual_backbone = SlidingWindowPoseBackbone()
        ...
        if self.args.np_loss:
            if self.args.mask_enc:
                self.tcloss = MaskedTCLoss(
                    N = self.args.np_sample, 
                    window_size = self.args.np_window,
                    add_gap = self.args.np_gap,
                )
            else:
                self.tcloss = TCLoss(
                    N = self.args.np_sample, 
    ...
    def forward(self, x, x_length, tgt=None, tgt_length=None, phase='train'):
    ...
    def loss_step(self, logits, labels, encoder_out = None, encoder_out_length = None):
        ...
        # add N-pair loss for encoder out
        if self.args.np_loss:
            if self.args.mask_enc:
                out_length = encoder_out_length.detach().cpu().numpy()
                tcloss = self.tcloss(encoder_out, out_length)
            else:
                tcloss = self.tcloss(encoder_out)
            loss = loss + self.args.np_alpha * tcloss
